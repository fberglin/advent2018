...
foreach $guard (keys %sleep) {
    foreach $minute (keys %{ $sleep{ $guard }->{ "frequency" } }) {
        if ($sleep{ $guard }->{ "frequency" }->{ $minute } > $maxFrequency) {
            $sleepiestGuard = $guard;
            $highestMinute = $minute;
            $maxFrequency = $sleep{ $guard }->{ "frequency" }->{ $minute };
        }
    }
}

print "Guard $sleepiestGuard sleeps most often at 00:$highestMinute, $maxFrequency times\n";
